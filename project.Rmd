...
The code chunk below creates a reaction diagram for our pathway using the amazing custom super exclusive package isocyclr! 
...
The variables defined above are now going to be used to assign numeric values to the "symbols" used in the reaction network created earlier.
...
params <- tibble(
      ...
      meS = 40, CO2 = 10, CH4 = 10, actCoA = 40, lipids = 10, obio = 10
      )
params
```
...
Answer: eps_CO2, it would have made the model even more complicated. It would effect acetyl-CoA and obio d13C values.
...
Answer: Pool size and flux only matter for residence times. By definition, the residence time is the total amount in the reservoir, divided by the total flux (in or out). Average length of time something will remain in a reservoir. Input increases, residence time decreases, input decreases, residence time increases. Larger reservoirs need more flux to reach steady-state faster.

### Question: In the grand scheme of things, does comparing high flux with low flux really matter? 
Answer: Nah, the system is gonna reach steady-state eventually.


# Model check to make sure everything is correct
```{r}
model %>% 
...
Answer: TRICK QUESTION. No difference.
...
Answer: Change in biomass yield moves the methane, other biomass, and lipids up, making them isotopically heavier. So changing the overall flux rate of the system doesn't impact fractionation of intermediates or products. However, changing the branching flux at specific nodes impacts the fractionation of the compounds that are connected to that node.
